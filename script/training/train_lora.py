...
from modeling.model_lora import load_lora, save_lora, apply_lora
...
if __name__ == '__main__':
    ...
    output_dir = "./model/lora_model"                             # 模型保存路径
    os.makedirs(output_dir, exist_ok=True)
    logging_dir = "./model/lora_model/logs"                       # 训练日志保存路径
    os.makedirs(logging_dir, exist_ok=True)
    logging_file = os.path.join(logging_dir, f"lora_log_{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}.txt")  # 训练日志文件路径，时间后缀改为日期-小时-分钟-秒格式
    train_data_dir = "./data/lora_data/train_data/lora.jsonl"      # 训练数据路径
    ...
    lora_config = \
        f"""模型训练超参数:
            ...
            (2) LOAR模型将被保存到: {output_dir} \n \
            (3) LOAR日志将被保存到: {logging_file}\n \
    ...
    with open(logging_file, "a", encoding="utf-8") as f:
        f.write(lora_config) # 写入模型训练超参数配置到日志文件中
    ...
    total_params_num = sum(p.numel() for p in tiaoyu_model.parameters())  # 总参数数量
    lora_params_num = sum(p.numel() for name, p in tiaoyu_model.named_parameters() if 'lora' in name)  # LoRA 参数数量
    print(f"LLM 总参数量: {total_params_num / 1e6:.3f} 百万")
    print(f"LoRA 参数量: {lora_params_num/ 1e6:.3f} 百万")
    print(f"LoRA 参数占比: {lora_params_num / total_params_num * 100:.2f}%")

    for name, param in tiaoyu_model.named_parameters():
        if 'lora' not in name:
    ...
    with open(logging_file, "a", encoding="utf-8") as f:        # 写入模型训练超参数配置到日志文件中
        f.write(f"LLM 总参数量: {total_params_num / 1e6:.3f} 百万 \n")
        f.write(f"LoRA 参数量: {lora_params_num/ 1e6:.3f} 百万 \n")
        f.write(f"LoRA 参数占比: {lora_params_num / total_params_num * 100:.2f}% \n")
    ...
    # 6. 初始化优化器(指定了优化器需要更新的参数(只对 LoRA 参数进行优化)、学习率)
    print('6. 初始化优化器...')
    ...
    for epoch in range(epochs):
        ...
        for step, (X, Y, loss_mask) in enumerate(train_loader):
            ...
            with ctx:
                ...
                loss = (loss * loss_mask).sum() / loss_mask.sum() + Y_hat.aux_loss
            ...
            if ((step + 1) % save_interval) == 0 | ((step + 1) == iter_per_epoch):
                tiaoyu_model.eval()
                lora_save_path = f'{output_dir}/lora_{tiaoyu_config.embed_dim}.pth'
                os.makedirs(os.path.dirname(lora_save_path), exist_ok=True)
                # 调用save_lora函数，仅保存LoRA参数. 可以使用 load_lora 函数加载LoRA参数(详见script/modeling/model_lora.py)
                save_lora(tiaoyu_model, lora_save_path)
                tiaoyu_model.train()
