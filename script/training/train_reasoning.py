...
if __name__ == "__main__":
    ...
    print("开始推理(Reasoning)学习TIAOYU模型...")
    ...
    # 2. 设置模型预训练参数
    print("2. 设置模型预训练参数...")
    pretrain_model_path = "./model/rlhf_model/rlhf_512.pth"       # 预训练模型路径
    output_dir = "./model/reasoning_model"                        # 模型保存路径
    os.makedirs(output_dir, exist_ok=True)
    logging_dir = "./model/reasoning_model/logs"                  # 训练日志保存路径
    os.makedirs(logging_dir, exist_ok=True)
    logging_file = os.path.join(logging_dir, f"reasoning_log_{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}.txt")  # 训练日志文件路径，时间后缀改为日期-小时-分钟-秒格式
    train_data_dir = "./data/reasoning_data/train_data/r1_mix_1024.jsonl"      # 训练数据路径
    ...
    sft_config = \
        f"""模型训练超参数:
            ...
            (2) Reasoning模型将被保存到: {output_dir} \n \
            (3) Reasoning日志将被保存到: {logging_file}\n \
    ...
    for epoch in range(epochs):
        ...
        # 获取思考、回复标签占位符的token ID
        start_of_think_ids = BPE_tokenizer('<think>').input_ids
        end_of_think_ids = BPE_tokenizer('</think>').input_ids
        start_of_answer_ids = BPE_tokenizer('<answer>').input_ids
        end_of_answer_ids = BPE_tokenizer('</answer>').input_ids
        ...
        for step, (X, Y, loss_mask) in enumerate(train_loader):
            ...
            with ctx:
                ...
                sp_ids = torch.isin(Y.view(-1), 
                                    torch.tensor(start_of_think_ids + end_of_think_ids + start_of_answer_ids + end_of_answer_ids).to(device))
                ...
                loss = (loss * loss_mask).sum() / loss_mask_sum + Y_hat.aux_loss
            ...
            if ((step + 1) % save_interval) == 0 | ((step + 1) == iter_per_epoch):
                tiaoyu_model.eval()
                ckp = f'{output_dir}/reasoning_{tiaoyu_config.embed_dim}.pth'
