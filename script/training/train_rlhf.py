...
from dpo_data_processing.dpo_data_load import DPODataset
...
if __name__ == '__main__':
    ...
    print("开始人类反馈强化学习(RLHF)TIAOYU模型...")
    ...
    # 2. 设置模型预训练参数
    print("2. 设置模型预训练参数...")
    pretrain_model_path = "./model/sft_model/sft_512.pth" # 预训练模型路径
    output_dir = "./model/rlhf_model"                             # 模型保存路径
    os.makedirs(output_dir, exist_ok=True)
    logging_dir = "./model/rlhf_model/logs"                       # 训练日志保存路径
    os.makedirs(logging_dir, exist_ok=True)
    logging_file = os.path.join(logging_dir, f"rlhf_log_{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}.txt")  # 训练日志文件路径，时间后缀改为日期-小时-分钟-秒格式
    train_data_dir = "./data/rlhf_data/train_data/rlhf_dpo.jsonl"      # 训练数据路径
    ...
    sft_config = \
        f"""模型训练超参数:
            ...
            (2) RLHF模型将被保存到: {output_dir} \n \
            (3) RLHF日志将被保存到: {logging_file}\n \
    ...
    print("(2) 初始化Refer模型...")
    refer_model = TIAOYU(tiaoyu_config)
    refer_model.load_state_dict(state_dict, strict=False)
    refer_model.eval()
    refer_model.requires_grad_(False)
    refer_model = refer_model.to(device)
    ...
    train_ds = DPODataset(data_path=train_data_dir, tokenizer=BPE_tokenizer, max_seq_len=tiaoyu_config.max_seq_len)
    ...
    # 7. 定义DPO损失函数
    def loss_fun(refer_probs, probs, loss_mask, beta):
        seq_lengths = loss_mask.sum(dim=1, keepdim=True)  # (batch_size, 1)
        refer_probs = (refer_probs * loss_mask).sum(dim=1) / seq_lengths.squeeze()
        probs = (probs * loss_mask).sum(dim=1) / seq_lengths.squeeze()

        # 将 chosen 和 rejected 数据分开
        batch_size = refer_probs.shape[0]
        chosen_refer_probs = refer_probs[:batch_size // 2]
        reject_refer_probs = refer_probs[batch_size // 2:]
        chosen_probs = probs[:batch_size // 2]
        reject_probs = probs[batch_size // 2:]

        pi_logratios = chosen_probs - reject_probs
        refer_logratios = chosen_refer_probs - reject_refer_probs
        logits = pi_logratios - refer_logratios
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            with ctx:
                # Refer模型的输出
                with torch.no_grad():
                    refer_outputs = refer_model(X)
                    refer_logits = refer_outputs.logits
                refer_log_probs = F.log_softmax(refer_logits, dim=2)
                refer_probs = torch.gather(refer_log_probs, dim=2, index=Y.unsqueeze(2)).squeeze(-1)
                refer_probs = refer_probs * loss_mask
                # 策略模型的输出
                outputs = tiaoyu_model(X)
                logits = outputs.logits
                log_probs = F.log_softmax(logits, dim=2)
                probs = torch.gather(log_probs, dim=2, index=Y.unsqueeze(2)).squeeze(-1)
                probs = probs * loss_mask
                # 计算DPO损失
                loss = loss_fun(refer_probs, probs, loss_mask, beta=0.1)
            ...
            if ((step + 1) % save_interval) == 0 | ((step + 1) == iter_per_epoch):
                tiaoyu_model.eval()
                ckp = f'{output_dir}/rlhf_{tiaoyu_config.embed_dim}.pth'
